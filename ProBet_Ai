...
<head>
    ...
    <style>
        ...
        select {
            background-color: #1e293b; color: white; border: 1px solid #334155; padding: 10px;
        ...
        .value-box:hover { transform: translateY(-2px); }

        .val-high { background-color: #15803d; color: white; border-color: #166534; } 
        .val-med { background-color: #ca8a04; color: #1a1a1a; border-color: #a16207; } 
        .val-low { background-color: #b91c1c; color: white; border-color: #991b1b; } 

        .lbl { font-size: 13px; text-transform: uppercase; font-weight: 800; opacity: 0.9; letter-spacing: 1px; }
        .res { font-size: 24px; font-weight: 900; margin: 5px 0; font-family: 'Teko', sans-serif; letter-spacing: 1px; }
        .dtl { font-size: 14px; font-weight: 700; display: inline-block; padding: 2px 8px; border-radius: 4px; }

        /* Loader */
        .loader {
            width: 18px; height: 18px; border: 2px solid #334155; border-bottom-color: #3b82f6;
...
<body class="min-h-screen pb-20">
        ...
        <div class="max-w-6xl mx-auto px-4 h-20 flex items-center justify-between">
            <div class="flex items-center gap-3">
                ...
                <div class="leading-none">
                    ...
                    <div class="text-[10px] text-slate-500 font-bold tracking-[0.3em]">FINAL CONNECT</div>
                </div>
            </div>
    ...
    <main class="pt-24 px-4 max-w-6xl mx-auto space-y-6">
        ...
        <!-- RESULTS (Value Finder) -->
        <div id="results" class="hidden pb-20">
            
            <div class="text-sm font-bold text-red-400 mb-3 border-b border-slate-800 pb-1 uppercase tracking-widest mt-8" id="title-falli">🛑 Analisi Falli</div>
            <div id="grid-falli" class="grid grid-cols-1 md:grid-cols-3 gap-4 mb-8"></div>

            <div id="sec-tiri" class="hidden">
                <div class="text-sm font-bold text-blue-400 mb-3 border-b border-slate-800 pb-1 uppercase tracking-widest mt-8">⚽ Analisi Tiri Totali</div>
                <div id="grid-tiri" class="grid grid-cols-1 md:grid-cols-3 gap-4 mb-8"></div>
                
                <div class="text-sm font-bold text-purple-400 mb-3 border-b border-slate-800 pb-1 uppercase tracking-widest mt-8">🎯 Analisi Tiri In Porta</div>
    ...
    <script>
        ...
        function switchLeague(l) {
            ...
            if (l === 'SERIE_A') {
                btnSa.className = "flex-1 py-2 text-sm font-bold rounded-lg bg-blue-600 text-white shadow-lg transition-all";
                btnLg.className = "flex-1 py-2 text-sm font-bold rounded-lg text-slate-400 hover:bg-slate-800 transition-all";
                boxTiri.classList.remove('hidden');
            } else {
                btnLg.className = "flex-1 py-2 text-sm font-bold rounded-lg bg-blue-600 text-white shadow-lg transition-all";
                btnSa.className = "flex-1 py-2 text-sm font-bold rounded-lg text-slate-400 hover:bg-slate-800 transition-all";
        ...
        async function loadData() {
            ...
            const L = CONFIG[CURRENT_LEAGUE];
            if (L.arb.length < 10) { status.innerHTML = "NO DATA"; return; }
            ...
            try {
                ...
                // Prende tutte le righe dalla 1 in poi (per sicurezza saltiamo la 0 se è intestazione)
                let arbStart = 0;
                ...
                }).filter(x => x && x.name.length > 2); // Filtra nomi corti/vuoti
                ...
                if (txtTiri.length > 50) {
                    ...
                    if(startIdx !== -1) {
                        // Mapping: 0:Sq, 1:PC, 2:TFC, 3:TSC, 4:TPFC, 5:TPSC, 6:PF, 7:TFF, 8:TSF, 9:TPFF, 10:TPSF
                        DB.tiri = rawData.slice(startIdx + 1).map(row => {
                            if(!row[0]) return null;
                            const pC = cleanNum(row[1]) || 1; 
                            const pF = cleanNum(row[6]) || 1; // FIX: Indice PF è 6 come da tuo script Python
                            return {
                                Team: cleanStr(row[0]),
                                // Totali
                                TF_C: cleanNum(row[2])/pC, TS_C: cleanNum(row[3])/pC,
                                TF_F: cleanNum(row[7])/pF, TS_F: cleanNum(row[8])/pF, // FIX Indici
                                // Porta
        ...
        function calculate() {
            ...
            if(CURRENT_LEAGUE === 'SERIE_A' && DB.tiri.length) {
                ...
                if(hT && aT) {
                    ...
                    // Cross: Attacco Casa vs Difesa Ospite
